...
//			pw.write("<script language='javascript'> alert('学生信息需要录入完整');window.document.location.href='pages/update.jsp' </script>");
//		}
//			else if(flag==false||flag2==false||flag3==false){
//			pw.write("<script language='javascript'> alert('学生电话号码或年龄格式或密码（6~16位数字加字母）格式不符合');window.document.location.href='pages/update.jsp' </script>");
//
			
			int num1=db.update(s);
			if(num1>0){
				resp.sendRedirect("SelectServlet");  
			}else{
				pw.write("<script language='javascript'> alert('未知原因跟新失败，请重试！');"
						+ "window.document.location.href='pages/update.jsp' </script>");
			}
		}
